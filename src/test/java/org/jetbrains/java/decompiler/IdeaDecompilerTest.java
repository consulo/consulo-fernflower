...
public class IdeaDecompilerTest extends LightCodeInsightFixtureTestCase
{
	...
	{
		String name = "/testData/" + getName().substring(4);
	...
	private void doTestStubCompatibility(VirtualFile root, @Nullable final String textPath)
	{
		...
		VfsUtilCore.visitChildrenRecursively(root, new VirtualFileVisitor()
		{
			...
			{
				if(file.getName().equals("Jps.class"))
				{
					return true;  // temp. fix
				}
				if(!file.isDirectory() && file.getFileType() == JavaClassFileType.INSTANCE && !file.getName().contains("$"))
				{
